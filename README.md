...
Voxtype keeps doing everything it does now. voxhud only replaces the overlay
and the bar icon:
...
- **The keys, not a guess.** The legend is read from your live Hyprland
  bindings, so it stays right if you ever rebind.
- **A bar icon that never disappears** (Omarchy's stock one hides itself
  while transcribing). Click it for your dictionary.

## Install

```sh
git clone <this repo> ~/Work/aashbury/voxhud
cd ~/Work/aashbury/voxhud
./install.sh
```

What that changes, all reversible with `./uninstall.sh`:

| Change | Why |
|---|---|
| `~/.config/omarchy/plugins/voxhud/` | the plugin |
| `voxhud` widget added to the bar (center) | enables the service, the HUD and the icon |
| Omarchy's `Dictation` indicator hidden | it goes invisible while processing; voxhud's icon replaces it |
| `osd.enabled = false` in Voxtype's config | turns Voxtype's own overlay off (daemon restarted once) |
...
voxhud dictionary          # replacements + vocabulary hints
voxhud dictionary add "oh marky" Omarchy
voxhud doctor              # check the install
```

Right-click the bar icon for the preview; left-click opens the dictionary.

## Settings

In Omarchy's bar settings (or `omarchy bar set voxhud <key> <value>`):
...
The bars show how far above the room's noise floor you are; a red **CLIP**
tag and red peak line mean the microphone input is saturating, which also
hurts transcription. That is the system's mic gain, not the HUD — lower it:

```sh
wpctl set-volume @DEFAULT_AUDIO_SOURCE@ 0.25    # WirePlumber remembers it
```

(On this laptop the ALSA `Capture` and `Internal Mic Boost` controls both
defaulted to +30 dB — 60 dB of gain — and clipped on room noise.)

## Known limits

- Silence takes Voxtype ~7 s (it retries with beam search) before it gives
  up; the HUD shows PROCESSING the whole time, then NOTHING HEARD.
- Push-to-talk vs toggle can't be told apart without routing the keys
  through voxhud (`bindingsMode: wrapper`, reserved), so the legend names both
  ways to insert.
...
Edit here → `./install.sh` → the shell hot-reloads → `voxhud demo tour`.
Errors: `journalctl --user -t omarchy-shell -f`.

The shell's hot reload notices the copy (one reload) but in practice keeps
running the old QML for both the service and the bar popup, so after editing
any `.qml` run `omarchy restart shell` (the bar blinks once). Scripts under
`bin/` are picked up on the next take without a restart.

## Uninstall

```sh
./uninstall.sh      # or: voxhud uninstall
```
